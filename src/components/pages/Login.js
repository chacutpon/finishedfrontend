...
const savePlayerData = async (email, name) => {
    try {
        ...
        startOfWeek.setDate(currentDate.getDate() - currentDate.getDay()); // หาวันแรกของสัปดาห์ (วันอาทิตย์)

        if (playerDoc.exists()) {
            const playerData = playerDoc.data();
            const lastLoginDate = new Date(playerData.lastLoginDate) || new Date(0);
            const loginCount = playerData.loginCount || 0;

            // ตรวจสอบว่าล็อกอินล่าสุดอยู่นอกสัปดาห์นี้
            if (lastLoginDate < startOfWeek) {
                await setDoc(playerRef, {
                    ...playerData,
                    lastLoginDate: currentDate.toISOString().split('T')[0],
                    loginCount: loginCount + 1
                });
                console.log("Player data updated successfully.");
            } else {
                console.log("Participation not counted for this week.");
            }
        } else {
            await setDoc(playerRef, {
                name: name,
