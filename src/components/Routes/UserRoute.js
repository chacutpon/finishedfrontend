...
//children พูดง่ายๆคือตัว  <HomepageUser/> ที่อยู่ภายใน <UserRoute> 
    return user.user && user.user.token && user.user.role === 'user'//checkว่า user(อันซ้ายก่อนจุดคือตัวแปลที่เรารับ)เข้าไปใน user ว่ามีข้อมูลไหม
    ? children //ถ้า true แสดง children(แสดง components ที่อยู่ข้างใน <HomepageUser/>)
    : (<h1>นี่คือส่วนของ User เท่านั้น!!!คุณไม่สามารถเข้าได้</h1>) //false
}
